...
class Life {
    ...
    Module = { PROPERTY:'PROPERTY', TALENT:'TALENT', EVENT:'EVENT', ACHIEVEMENT:'ACHIEVEMENT', CHARACTER:'CHARACTER' }
    Function = { CONDITION:'CONDITION', UTIL:'UTIL' }

    #property; #event; #talent; #achievement; #character;
    #triggerTalents; #defaultPropertyPoints; #talentSelectLimit;
    #propertyAllocateLimit; #defaultPropertys; #specialThanks; #initialData;
    #pendingChoice = null;

    async initial(i18nLoad, commonLoad) {
        const [age, talents, events, achievements, characters, specialThanks] = await Promise.all([
            i18nLoad('age'), i18nLoad('talents'), i18nLoad('events'),
            i18nLoad('achievement'), i18nLoad('character'), commonLoad('specialthanks'),
        ]);
    ...
    config({ defaultPropertyPoints=20, talentSelectLimit=3,
        propertyAllocateLimit=[0,10], defaultPropertys={},
        talentConfig, propertyConfig, characterConfig } = {}) {
    ...
    check(condition) { return fCondition.checkCondition(this.#property, condition); }
    clone(...args)    { return util.clone(...args); }
    ...
    start(allocation) {
        for(const key in allocation) this.#initialData[key] = util.clone(allocation[key]);
    ...
    next() {
        if(this.#pendingChoice) return { awaitingChoice: true, choices: this.#pendingChoice };
        const {age, event, talent} = this.#property.ageNext();
        const talentContent = this.doTalent(talent);
        const eventResult   = this.doEvent(this.random(event));
        if(eventResult.awaitingChoice) {
            return { age, content: [...talentContent, eventResult.content], awaitingChoice: true, choices: eventResult.choices };
        }
        const isEnd = this.#property.isEnd();
        const content = [talentContent, eventResult].flat();
    ...
    choose(choiceIndex) {
        ...
        const choice = this.#pendingChoice[choiceIndex];
        this.#pendingChoice = null;
        if(!choice) return null;
        const result = this.doEvent(choice.next);
        if(result.awaitingChoice) return result;
        return result;
    }

    get awaitingChoice() { return this.#pendingChoice !== null; }
    ...
    doTalent(talents) {
        ...
        for(const talentId of talents) {
            ...
            contents.push({ type: this.PropertyTypes.TLT, name, grade, description: this.format(description) });
            if(!effect) continue;
    ...
    doEvent(eventId) {
        const { effect, next, choices, description, postEvent, grade } = this.#event.do(eventId);
        ...
        // 玩家选择事件 — 暂停等待输入
        if(choices) {
            // 过滤掉有 require 条件但不满足的选项
            const available = choices.filter(c => !c.require || this.check(c.require));
            if(available.length > 0) {
                this.#pendingChoice = available;
                return { content, awaitingChoice: true, choices: available };
            }
            // 如果所有带条件的选项都不满足，用第一个选项作为默认
            this.#pendingChoice = choices.slice(0, 1);
            return { content, awaitingChoice: true, choices: this.#pendingChoice };
        }
    ...
    random(events) {
        return util.weightRandom(events.filter(([eventId]) => this.#event.check(eventId, this.#property)));
    }

    talentRandom() {
        return this.#talent.talentRandom(this.lastExtendTalent, this.#getPropertys(this.PropertyTypes.TMS, this.PropertyTypes.CACHV));
    }
    ...
    talentExtend(talentId) { this.#property.set(this.PropertyTypes.EXT, talentId); }
    exclude(talents, exclusive) { return this.#talent.exclude(talents, exclusive); }
    generateUnique() { this.#character.generateUnique(); }

    #getJudges(...types)   { return util.getListValuesMap(types.flat(), key => this.#property.judge(key)); }
    #getPropertys(...types){ return util.getListValuesMap(types.flat(), key => this.#property.get(key)); }

    format(description) {
        return `${description}`.replaceAll(/\{\s*[0-9a-zA-Z_-]+\s*?\}/g, match => this.#format(match));
    }
    ...
    get lastExtendTalent()        { return this.#property.get(this.PropertyTypes.EXT); }
    get summary() {
    ...
    get achievements() {
        ...
        return this.#achievement.list(this.#property).sort(({id:a,grade:ag,hide:ah},{id:b,grade:bg,hide:bh}) => {
            a = ticks[a]; b = ticks[b];
            if(a&&b) return b-a;
            if(!a&&!b) { if(ah&&bh) return bg-ag; if(ah) return 1; if(bh) return -1; return bg-ag; }
