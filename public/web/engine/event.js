class Event {
    ...
    do(eventId) {
        const { effect, branch, choices, event: description, postEvent, grade } = this.get(eventId);
        // 有玩家选择时，返回 choices 供 UI 处理
        if(choices) return { effect, choices, description, grade };
        if(branch)
